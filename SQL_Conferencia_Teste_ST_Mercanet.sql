...
/*       NVL((SELECT SUM(AL.alfvtr)
       ...
       ARQ.id_produto,
       SA.saan8 AS CLIENTE,
       SA.savr01 AS PED_MERC,
       SD.sddoco AS PED_JDE,
       SA.saptc AS COND_PGTO,
       SA.samcu AS EMPRESA,
       trim(SB.sblitm) AS PRODUTO,
       PEDI.db_pedi_qtde_solic AS QTDE_SOLIC_OL,
       PEDI.db_pedi_qtde_atend AS QTDE_ATEND_OL*/
  ...
  left join mercanet_qa.db_pedido_prod@dc12.drogacenter.com.br PEDI
    on PEDI.db_pedi_pedido = PDIT.db_pdit_pedido
      --and PEDI.db_pedi_sequencia = PDIT.db_pdit_pedi_seq
 ...
 where TO_NUMBER(ARQ.id_arquivo) BETWEEN 1 and 90
   and ((mod(nvl(SD.sdlnid,0),1000) = 0 and mod(nvl(SDEF.sdlnid,0),1000) = 0))
 order by TO_NUMBER(id_arquivo), TO_NUMBER(id_produto);
 
 /*SELECT B.FDLITM,b.fdaexp,b.fduorg, fdaexp/fduorg,edan03 / fduorg, sdlprc PRECO_DE_LISTA , sdaexp/sdsoqs, sdemcu,sduprc,sdlprc , a.* FROM QADTA.F555511 a, qadta.f7611b b , qadta.f4211 cWHERE edbnnf = fdbnnfand edbser = fdbserand edn001 = fdn001and fddoco = 10175295 and fdlnid = edlnidand fddoco = sddoco 
 and fdlnid = sdlnid*/
/*
SELECT 'QATDTA ATUAL', A.* FROM QADTA.F7608B A
WHERE TBITM = 2402
AND TBSHST = 'PR'
AND TBADDS = 'PR'
AND TBAC76 = '*'
UNION ALL
SELECT 'PRODDTA ATUAL' ,A.* FROM PRODDTA.F7608B@DC01.DROGACENTER.COM.BR A
WHERE TBITM = 2402
AND TBSHST = 'PR'
AND TBADDS = 'PR'
AND TBAC76 = '*'
UNION ALL
SELECT 'QADTA ANTES', A.* FROM DSALES.F7608B_TESTE_ST_MERC A
WHERE TBITM = 2402
AND TBSHST = 'PR'
AND TBADDS = 'PR'
AND TBAC76 = '*'
