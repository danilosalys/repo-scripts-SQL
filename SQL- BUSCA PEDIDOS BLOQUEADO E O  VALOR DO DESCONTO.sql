...
select A.DB_EDIP_VAN,
       ...
       CASE
         WHEN (C.DB_EDII_PERC_DCTO > 0) THEN
          SUM(C.DB_EDII_QTDE_VDA * G.DB_PRECOP_VALOR) -
          (SUM(C.DB_EDII_QTDE_VDA * G.DB_PRECOP_VALOR) * C.DB_EDII_PERC_DCTO) / 100
         ELSE
          SUM(C.DB_EDII_QTDE_VDA * G.DB_PRECOP_VALOR)
  ...
  from DB_EDI_PEDIDO     A,
       ...
       DB_PRECO_PROD     G,
       DB_LABORATORIO    H
 where A.db_edip_pedmerc is null
   AND A.DB_EDIP_LOTE BETWEEN 7854 AND 7858 --NUMERO DO LOTE
